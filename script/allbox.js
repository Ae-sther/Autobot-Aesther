...
module.exports.run = async function({ api, event, args }) {
		...
		switch (args[0]) {
				case "all": {
						...
						msg += `--Page ${page}/${numPage}--\nDy ${this.config.name} page number/all\n\n`;

						api.sendMessage(msg + '🎭Reply Out, Ban, Unban, Del[data] the order number to Out, Ban, Unban, Del[data] that thread!', threadID, (e, data) =>
								api.sendMessage({
										body: msg + '🎭Reply Out, Ban, Unban, Del[data] the order number to Out, Ban, Unban, Del[data] that thread!',
										attachment: data.attachment
								}, threadID, (e, info) => this.handleReply({ api, event, args, handleReply: { author: event.senderID, messageID: info.messageID, groupid, groupName, type: 'reply' } })));
						break;
				}
