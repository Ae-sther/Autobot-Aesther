...
const name = info[event.senderID].name
    ...
    if(msgData[event.messageID].attachments.length === 0) {
        api.sendMessage(`📅${name}📅\n(・_・ヾ𝚍𝚎𝚕𝚎𝚝𝚎𝚍 𝙼𝚜𝚐\n[𝗖𝗢𝗡𝗧𝗘𝗡𝗧]: ${msgData[event.messageID].body}`, event.threadID)  
      ...
      for (const item of msgData[event.messageID].attachments) {
       let { data } = await axios.get(item.url, {responseType: "arraybuffer"})
          ...
          del.push(`./script/cache/${item.filename}.jpg`)
                    ...
                    api.sendMessage({body:`📅${name}📅\n(・_・ヾ𝚍𝚎𝚕𝚎𝚝𝚎𝚍 𝙿𝙸𝙲\n[𝗖𝗢𝗡𝗧𝗘𝗡𝗧]: ${msgData[event.messageID].body}`, attachment: photo}, event.threadID, () => {
...
api.sendMessage({body:`📅${name}📅\n(・_・ヾ𝚍𝚎𝚕𝚎𝚝𝚎𝚍 𝚅𝚘𝚒𝚌𝚎\n[𝗖𝗢𝗡𝗧𝗘𝗡𝗧]: ${msgData[event.messageID].body}`, attachment: fs.createReadStream('./script/cache/audio.mp3')}, event.threadID, () => {
...
api.sendMessage({body:`📅${name}📅\n(・_・ヾ𝚍𝚎𝚕𝚎𝚝𝚎𝚍 𝙶𝙸𝙵\n[𝗖𝗢𝗡𝗧𝗘𝗡𝗧]: ${msgData[event.messageID].body}`, attachment: fs.createReadStream('./script/cache/animated_image.gif')}, event.threadID, () => {
